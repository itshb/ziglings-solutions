...
const TripItem = union(enum) {
    ...
    fn printMe(self: TripItem) void {
        switch (self) {
            ...
            .path => |p| print("--{}->", .{p.dist}), // Fix: remove s in the format (printing int, not a string)
        }
    }
};
...
const HermitsNotebook = struct {
    entries: [place_count]?NotebookEntry = .{null} ** place_count, // ** == array repetition operator (insert null n times)
    next_entry: u8 = 0,
    end_of_entries: u8 = 0,
    ...
    fn getEntry(self: *HermitsNotebook, place: *const Place) ?*NotebookEntry {
        for (&self.entries, 0..) |*entry, i| {
            ...
            if (place == entry.*.?.place) return &entry.*.?; // Fix: unwrap & return the address
        }
        return null;
    }
    ...
    fn getNextEntry(self: *HermitsNotebook) *const NotebookEntry {
        defer self.next_entry += 1; // Kind of sick being able to do stuff after returning, but also equally frightening..
